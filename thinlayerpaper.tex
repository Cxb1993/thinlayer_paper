...
%In this work to address thin layer problem and other related instability and accuracy problems, we implemented a combination of Heuristic approaches, Phase filed and level set methods and compared the obtained results with experimental and field data for granular flow. Phase field and level set methods are very well-known Eulerian interface capturing methods that to the best knowledge of authors are applied to geophysical flow.\newline
In this paper, our approach to address the resulted problems from the thin layer problem (problem 1-4 discussed in introduction section), consists of two parts. The first part is to find the interface of the flow, and the next part is to do some steps based on the result of the first part to resolve the aforementioned issues.
To capture the interface we study three different methods. The first one is to use a very small dimensionless threshold, which we call it GEOFLOW\_TINY, to distinguish between wet and dry areas. The other method that we implement to capture the interface is phase field. \newline

{ \huge I have to rewrite this part}
\newline

 that is resulted from the thin layer problem and discussed in  two do some  we proposed a new multifaceted Heuristic approach and also tested the effectiveness of Level set and Phase field methods to mitigate the thin layer problem difficulties. Our approach in this work consist of two parts.  for numerical solution of SW equations. To the best knowledge of authors, the two later methods  are for the first time applied to geophysical flow. Although these methods have been used for granular type SW flows, most of the conditions holds and the results are applicable for other type of SW flows.  We verified the numerical results with experimental result of a granular SW flow over an inclined plane ends to a horizontal surface , as well as the field data for Colima volcano.
...
In level set part, we followed the instructions in paper\cite{Sussman1994} with the re-initialization scheme introduced in Adalsteinsson's work \cite{Adalsteinsson1995}. To solve both of these hyperbolic equations we used a first order accurate solver.
...
\item They are at or within a few cells near the boundary.
% of any of four small, non-dimensional flow depths (multiples of {\rm\verb1GEOFLOW_TINY1}).
\end{enumerate}
The former condition allows for accurate capture of sharp changes in state variables. The latter results in banded regions of the flow separated into ``rings'' of maximally refined buffer cells.  This limits artificially high transportation of material from the larger depth bands to the lower depth
bands. In solution section, we explain how we select the elements near the boundary for refinement for each method of interface capturing that we use. 
%The outermost ring is at a flow depth of {\rm\verb1GEOFLOW_TINY1}.
The rings are a number of cells wide equal to or greater than the number of iterations between mesh adaptations (we are not doing mesh adoption at each time step).  
This guarantees that material will only flow into maximally refined dry cells at the wet-dry front.  
%However, material with flow depth less than {\rm\verb1GEOFLOW_TINY1} may be left outside the outermost band of buffer cells by the passing flow.  
%The rings of the later  
%buffer cells strategy decreases the numerical wicking (Problem 
%\ref{problemwicking} from the thin layer problems in section introduction of this paper) both within the 
%flow and at the boundary.  When combined with thresholding, it is sufficient to decrease Problems 
...
%\ref{problemunstable}.
{\huge here I have to rewrite, I have to say we use GEOFLOW\_TINY in all of the methods, I have to also add an adative section to each part of solution}
...
%   \hfill
  ...
  \label{odinary}
\end{figure}
{\huge it would be very good if I can plot data of experiment for area and perimeter}
