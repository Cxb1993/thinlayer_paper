...
This formulation is not mass conservative. Therefore, to conserve the mass a Lagrange multiplier in form of $(\varphi^2-1)\xi(t)$ is added as a source term to 
the right hand side of equation \eqref{allencahn}, where $\xi(t)$ is the Lagrange multipler to be determined. 
The reason that we select this form for the Lagrange multiplier is because we do not want to change $\varphi$ on the bulk region as well as when on the interface i.e when $\varphi=\pm1$. 
The modified Allen-Cahn equation is now:
\begin{equation} 
        \label{allencahn_mod}
        \frac{\partial \varphi }{\partial t} + \overrightarrow{V}\cdot \nabla \varphi = 
        \gamma (\bigtriangleup\varphi -F'(\varphi)+ (\varphi^2-1)\xi(t)),
\end{equation}
where the constrain to be satisfied is:
\begin{equation} 
        \label{constrain}
...
\end{equation}
The first term in above relation will be neglected by applying the Gauss' theorem with consideration of the boundary conditions($
...
\begin{equation}
        ...
        \int_\Gamma h \gamma \nabla \varphi \ ds = 0.
\end{equation}
To ensure that constraint \eqref{constrain} is satisfied $\xi(t)$ must be given by:
\begin{equation} 
        \label{eta_cont}
...
The phase-field time-evolution equation, Eq. \eqref{allencahn_mod}, is updated using a semi-implicit scheme. The non-linear terms are approximated using an explicit method
while the linear, Laplacian term is solved implicitly:
\begin{equation}
	\frac{\varphi^{n+1}-\varphi^n}{\bigtriangleup t}= \gamma \nabla .\nabla \varphi^{n+1}+\gamma (-F'(\varphi^n)+\varphi^n (1-(\varphi^n)^2)\ \xi(t^n)).
\end{equation}
Note that the Lagrange multiplier is determined using the explicit information available at time $t^n$.
The resulting linear system is solved using the GMRES algorithm available in the 
PETSc \cite{petsc-user-ref} library.
As GMRES only needs the result of matrix-vector multiplication, we used the matrix-free version to compute the laplacian term to decrease the memory cost of implicit solver.
...
the computational domain. The level set variable, $\varPsi (X,t)$, implicitly represents the interface. In this method, $\varPsi (X,t)$ is a signed distance function which 
means it is zero on the interface, and its absolute value denotes the distance of a spatial point to the interface, 
and its sign is positive outside of the interface and negative inside of the interface. 
The movement of the interface is captured by advecting the level set due to the underlying fluid flow field, $overrightarrow{V}$:
\begin{equation}\label{levelseteq2}
        \frac{\partial \varPsi}{\partial t} + \overrightarrow{V}\cdot\nabla \varPsi = 0.
\end{equation}
