...
In figure \ref{x_extent} the extent of flow in X direction was plotted. This plot shows the length of flow during its travel. The maximum of the flow length happens when the flow reaches the horizontal plane. All of the methods are successful in predicting the time that flow reaches the horizontal plane, but like interface results (figure \ref{Pile_height_contour}) the heuristic methods overpredict the length of the flow, and the Phase Field and the Level Set results are better match with the experiment. 

Width of the flow or extent of the flow in Y direction can be seen in figure \ref{y_extent}. There is more difference between the experiment and the Eulerian interface capturing methods results in this figure than the other results, and based on our previouse work \cite{Patra2006}, we believe the main reason of this difference is due to the accuracy of the solver. In that work, we showed with more accurate simulation with Discontinuous Galerkin method we can better capture the width of the flow.
...
path of flow, so a good performance of any of the interface capturing methods for this case promises a reliable method for other volcanoes. Unfortunately, there are very rare cases that data during the eruption available for a volcano. So we cannot compare the numerical results with the field data during the time of the event. Usually for this application, the deposit of the volcanic events is used to find the outline of the flow and to validate the numerical results.

In figure \ref{Colimapic} we compared the results of the three methods with the outline of the flow from the eruption obtained from the field data \cite{Rupp2006}. In numerical simulations the outline of the flow is the boundary between wet and dry areas of the domain for whole the simulation. In other words, the points at least flow pass them once are inside the outline, and rest of the points are located outside of the outline of the flow. 

Here like what we did in inclined plane case, we use the same definition for the interface for the three methods. Thus $\phi=0$ and $\varPsi=0$ for the Phase Field and the Level Set methods respectively and $h=\sqrt[3]{Vol} \times GEOFLOW\_TINY $ for the heuristic method is used to discrementae between wet and dry areas.
...
In figure \ref{colima_outline} you can see the plotted outline of the eruption on Colima volcano. To be able to compare the outline of each numerical methods with this outline we plot it in all of the results in figure  \ref{Colimapic} with the same color.
...
Results of figure \ref{Colimapic} shows that all of the method are successful to control the thin layer problem. The length of the flow has been captured in all of the methods very good which is very important for generating the hazard maps. Width of the flow is also reasonable in all of the three methods, though the Eulerian methods are more successful. However the heuristic method is the only method that captured the channeling of the flow that can be seen in the field data, but there are two small channels in the heuristic method which are not present in the field outline.
...
The computational comparison of the methods for Colima volcano is shown in figure \ref{compinc}. We used 16 x 2.27GHz Intel(R) Xeon(R) L5520 for this case. Figure \ref{col_amr} shows the number of the elements during the simulation for the three methods.  This figures shows the heuristic method and level set have higher number of elements than the phase field method, but requires more number of iteration to perform 1 hr physical time simulation. The right figure (figure \ref{comp_col}) shows the required time for the simulation. In this figure we can see all of the methods take almost same amount of time for computation which means that although the Level Set and the Phase Field methods are more computationally expensive per time step, but lower number of elements and less number of iterations help them to have the same computational cost as the heuristic method.  
