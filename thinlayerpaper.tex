...
To this aim, Eglit and Sveshnikova \cite{eglit1980mms} modified the depth-averaged Saint Venant equations to simulate granular snow avalanches, and almost a decade later Savage and Hutter\cite{SavageHutter1989} to adapt the depth averaged Saint Venant equations to accommodate  geophysical mass flow. 
...
More clearly, the governing equations are just valid in wet areas, so we used a way to discriminate between wet and dry areas in the numerical simulation. 
This problem is usually called the {\bf wetting and drying (WD)} or {\bf Thin-Layer} problem in SW context, and the lack of a solution for this problem leads to the formation of a non-physical thin layer in the domain.
...
\item \label{problemtoofast}
      Non-physically high flow speeds.  Since the solution of SWE are momentum, $\{hV_x,hV_y\}$, to find velocities, $\{V_x,V_y\}$ used in solution process, the momentum must be divided by flow depth, $h$, which causes a numerical error in an already small $h$, and result in overly large velocities.
...
points (particles) that represents the interface. These points move with a velocity vector field which comes 
...
The last method that we will overview here is phase field method. Phase field is another Eulerian interface capturing scheme that the interface is implicitly related to the order parameter which shows the phase variation on the domain. 
...
will make the later form much easier to implement. As a result people first try to use the second one and then if 
they not succeed, they will proceed to the other one.
...
Based on our best knowledge no report was published earlier that uses this method for capturing the interface in SW.\newline

%=================== Introduction of next parts of paper ==========================
In this work, we used a combination of the first group of approaches and then compared these results with the results of the phase field and the level set methods. We tried to stick to the basic methods, as long as the quality of the result was reasonable and it does not increase the computational cost significantly.\newline
...
they do not have a rigorous physical basis, the obtained interface. 
On other hand, the capturing interface methods have stronger mathematical and physical structure, and they can be coupled with the other equations to be employed for a more complicated case. 
However, these types of approaches are more computationally expensive, and are harder to implement and couple with the basic solver
