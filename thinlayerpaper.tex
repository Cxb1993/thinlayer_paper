...
In this paper, we proposed a new multifaceted Heuristic approach and also tested the effectiveness of Level set and Phase field methods to mitigate the thin layer problem difficulties for numerical solution of SW equations. Although these methods have been used for granular type SW flows, we believe that most of it is also applicable for other type of SW flows.  We verified the numerical results with experimental result of a granular SW flow over an inclined, and the field data for Colima volcano.
In Eulerian interface capturing methods we tried to stick to the basic methods, and decrease the computational cost with using smart solutions if applicable. For example, in Level set method we used narrow band re-initialization to just update the value of order parameter near the interface, and in Phase field method we use operator splitting and matrix free method. Detail of these steps can be find in the corresponding section (sections \ref{phase field},\ref{level set}).

The basic procedure in the heuristic approach is: First, use a very small threshold to discriminate between wet and dry cells, then divide the cells into three groups of wet, dry and partially wet cells.
...
In the level set part, the basic level set was implemented \cite{} with the re-initialization scheme introduced by \cite{} 
for every 20 time steps. To solve both of these hyperbolic equations we used a first order accurate solver.\newline
...
used, which allows one half to be solved by an implicit scheme, and the rest of the terms were updated in the default explicit 
integrator.\newline
