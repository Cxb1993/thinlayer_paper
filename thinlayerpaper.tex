...
which are updated implicitly. The size of time step is based on CFL condition for the explicit scheme. \red{Do you mean that the time step is chosen based on the velocity?}
\begin{align}
...
PDE based reinitialization is easy for parallelization, though it has been known to induce mass loss. Chopp's suggests a bicubic interpolation for the points near the interface \cite{Chopp2001},
...
The experimental test results were reported in \cite{AmyWebb2004}, and we have used them to verify our different numerical results with them in our previous works.
